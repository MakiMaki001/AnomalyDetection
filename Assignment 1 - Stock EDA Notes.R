...
#Function with all stocks from the article
...
#Filtering for above 10% Growth per day
TLRY2 %>%
  mutate(pdchg = (TLRY.Adjusted - TLRY.Open)/TLRY.Open*100)%>%
  filter(pdchg > 10)

#Filtering for above 10% Growth per day and above 1 sd of movement (4835917)
TLRY2 %>%
  mutate(pdchg = (TLRY.Adjusted - TLRY.Open)/TLRY.Open*100)%>%
  filter(pdchg > 10) %>%
  filter(TLRY.Volume>4835917)
...
#Volume divided by absolute value of growth per day
TLRY2 %>%
  mutate(pdchg = (TLRY.Adjusted - TLRY.Open)/TLRY.Open*100, volpdchg = TLRY.Volume/abs(pdchg))

#Filtering for high volumes on days that had more than a 10% increase in price
TLRY2 %>%
  mutate(prchg = TLRY.Adjusted - lag(TLRY.Adjusted), prcchg = prchg / lag(TLRY.Adjusted) * 100)%>%
  filter(!between(prchg, -10, 10))%>%
  mutate(volpdchg = TLRY.Volume/abs(prcchg))

#Adding volatility
TLRY2 %>%
  mutate(vol = (TLRY.High- TLRY.Low)/TLRY.Open)

#Filtering for days that have high volatility
TLRY2 %>%
  mutate(vol = (TLRY.High- TLRY.Low)/TLRY.Open)%>%
  filter(vol>.10)

#Filtering for days that have high volatility and volume above 1 sd (4835917)
TLRY2 %>%
  mutate(vol = (TLRY.High- TLRY.Low)/TLRY.Open)%>%
  filter(vol>.10)%>%
  filter(TLRY.Volume>4835917)

#Filtering for above 10% Growth per day, above 1 sd of movement (4835917), and high volatility
TLRY2 %>%
  mutate(pdchg = (TLRY.Adjusted - TLRY.Open)/TLRY.Open*100, vol = (TLRY.High- TLRY.Low)/TLRY.Open)%>%
  filter(pdchg > 10) %>%
  filter(TLRY.Volume>4835917)%>%
  filter(vol>.10)
  

mean(TLRY2$TLRY.Volume)
max(TLRY2$TLRY.Volume)
sd(TLRY2$TLRY.Volume)
plot(TLRY2$TLRY.Volume)
